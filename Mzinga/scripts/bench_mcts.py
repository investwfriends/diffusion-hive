"""MCTS self-play benchmark for Mzinga optimization work.

Runs N self-play games with MCTS(num_sims=S) and reports throughput.
Used to measure impact of micro-optimizations in src/mzinga/core/.

This actually exercises the MCTS search path: per move, MCTS does
`num_sims` simulations, each calling get_valid_moves and trusted_play/undo
multiple times. That's the hot path we're optimizing.

Usage:
    uv run python scripts/bench_mcts.py [num_games] [num_sims] [hidden_dim] [num_blocks]
"""
...
def main() -> int:
    ...
    for g in range(num_games):
        ...
        sims = moves * num_sims
        per_game.append((moves, dt, sims / max(dt, 1e-6)))
        total_moves += moves
        total_sims += sims
        total_time += dt
    ...
    print()
    print(f"Total:     {total_moves} moves, {total_sims} sims in {total_time:.2f}s")
